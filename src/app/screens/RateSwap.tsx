  ...
  if (!convo) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#F5F0E8] gap-4">
        <p className="text-[#3D3530]/60">Swap not found</p>
  ...
  return (
    <div className="h-full flex flex-col bg-[#F5F0E8]">
      <div className="bg-white border-b border-[#3D3530]/10 px-4 py-3.5 flex items-center gap-3 shrink-0">
        <button onClick={() => navigate("/inbox")} className="p-2 hover:bg-[#F5F0E8] rounded-full transition-colors">
          <ChevronLeft className="w-5 h-5 text-[#3D3530]" strokeWidth={1.5} />
        </button>
        <h1 className="font-heading text-xl text-[#3D3530]">Rate your swap</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain px-6 py-8 flex flex-col items-center">
        ...
        />
        <p className="text-[#3D3530] mb-1">
          How was swapping with <span className="font-medium">{partner?.name ?? "them"}</span>?
        </p>

        {alreadyRated ? (
          <p className="text-[#3D3530]/60 mt-6">You've already reviewed this swap. 🙌</p>
        ) : (
          <>
            <div className="flex gap-1.5 mt-5 mb-1">
              {[1, 2, 3, 4, 5].map((i) => (
                <button key={i} onClick={() => setRating(i)} className="p-1 active:scale-90 transition-transform">
                  <Star
                    className={`w-9 h-9 ${i <= rating ? "text-[#C2794A] fill-[#C2794A]" : "text-[#3D3530]/20"}`}
                    strokeWidth={1.5}
                  />
                </button>
              ))}
            ...
            <textarea
              ...
              className="w-full bg-white rounded-2xl px-4 py-3 text-[#3D3530] placeholder-[#3D3530]/40 focus:outline-none focus:ring-2 focus:ring-[#6B7A5C] resize-none"
            />
